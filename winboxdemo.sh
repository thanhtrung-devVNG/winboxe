...
if [[ "$WEB_UI_MODE" == "1" ]]; then
    ...
    _check_port() { python3 -c "import socket;s=socket.socket();s.bind(('127.0.0.1',$1));s.close()" 2>/dev/null; }
    _orig_port="$WEB_PORT"
    while ! _check_port "$WEB_PORT"; do
        WEB_PORT=$((WEB_PORT + 1))
        if [[ $WEB_PORT -gt ${_orig_port}+20 ]]; then
            echo -e "${R}✘${W}  Không tìm được port trống"
            exit 1
        fi
    done
    if [[ "$WEB_PORT" != "$_orig_port" ]]; then
        echo -e "${Y}⚠${W}  Port $_orig_port đã bị chiếm, chuyển sang port $WEB_PORT"
    fi
...
<style>
:root{--bg:#0f0f1a;--card:#1a1a2e;--hover:#252542;--accent:#00d4aa;--accent-dim:#00d4aa33;--text:#e0e0e0;--dim:#888;--border:#2a2a4a;--red:#ff4757;--yellow:#ffa502;--blue:#3742fa}
...
header h1{font-size:2.5rem;background:linear-gradient(90deg,var(--accent),#00b4d8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:10px}
header p{color:var(--dim);font-size:1.1rem}
.status-bar{display:flex;gap:15px;justify-content:center;margin-bottom:25px;flex-wrap:wrap}
.status-pill{background:var(--card);border:1px solid var(--border);padding:8px 20px;border-radius:20px;font-size:.9rem;display:flex;align-items:center;gap:8px}
.status-pill .dot{width:8px;height:8px;border-radius:50%;background:var(--accent);animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
.section-title{font-size:1.3rem;margin:25px 0 15px;display:flex;align-items:center;gap:10px}
.section-title span{color:var(--accent)}
.os-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:15px;margin-bottom:30px}
.os-card{background:var(--card);border:2px solid var(--border);border-radius:16px;padding:20px;cursor:pointer;transition:all .3s;position:relative;overflow:hidden}
.os-card:hover{border-color:var(--accent);transform:translateY(-3px);box-shadow:0 10px 30px rgba(0,212,170,.1)}
.os-card.selected{border-color:var(--accent);background:var(--accent-dim)}
.os-card .os-icon{font-size:2.5rem;margin-bottom:10px}
.os-card h3{font-size:1.1rem;margin-bottom:5px}
.os-card .os-desc{font-size:.85rem;color:var(--dim)}
.os-card .os-badge{position:absolute;top:15px;right:15px;background:var(--accent);color:var(--bg);padding:3px 10px;border-radius:10px;font-size:.7rem;font-weight:bold}
.config-panel{background:var(--card);border:2px solid var(--border);border-radius:16px;padding:25px;margin-bottom:20px;display:none}
.config-panel.active{display:block;animation:slideDown .3s}
@keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
.config-tabs{display:flex;gap:10px;margin-bottom:20px;border-bottom:1px solid var(--border);padding-bottom:10px}
.config-tab{padding:10px 25px;border-radius:10px;cursor:pointer;border:none;background:transparent;color:var(--dim);font-size:1rem;transition:all .2s}
.config-tab:hover{color:var(--text)}
.config-tab.active{background:var(--accent);color:var(--bg);font-weight:bold}
.preset-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;margin-bottom:20px}
.preset-card{background:var(--bg);border:1px solid var(--border);border-radius:12px;padding:18px;cursor:pointer;transition:all .2s;text-align:center}
.preset-card:hover{border-color:var(--accent)}
.preset-card.selected{border-color:var(--accent);background:var(--accent-dim)}
.preset-card .preset-name{font-weight:bold;font-size:1rem;margin-bottom:8px;color:var(--accent)}
.preset-card .preset-specs{font-size:.85rem;color:var(--dim);line-height:1.6}
.custom-config{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:20px}
.input-group{display:flex;flex-direction:column;gap:8px}
.input-group label{font-size:.9rem;color:var(--dim);font-weight:500}
.input-group input,.input-group select{background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:12px 15px;color:var(--text);font-size:1rem;outline:none;transition:border-color .2s}
.input-group input:focus,.input-group select:focus{border-color:var(--accent)}
.input-group .hint{font-size:.8rem;color:var(--dim)}
.auth-section{background:var(--bg);border-radius:12px;padding:20px;margin-top:20px;border:1px solid var(--border)}
.auth-section h4{color:var(--accent);margin-bottom:15px;display:flex;align-items:center;gap:8px}
.auth-grid{display:grid;grid-template-columns:1fr 1fr;gap:15px}
.summary-box{background:linear-gradient(135deg,var(--accent-dim),transparent);border:1px solid var(--accent);border-radius:12px;padding:20px;margin-top:20px}
.summary-box h4{color:var(--accent);margin-bottom:12px}
...
.btn-group{display:flex;gap:15px;margin-top:25px;justify-content:center}
.btn{padding:14px 40px;border-radius:12px;border:none;font-size:1.1rem;font-weight:bold;cursor:pointer;transition:all .3s;display:flex;align-items:center;gap:10px}
.btn-primary{background:linear-gradient(135deg,var(--accent),#00b4d8);color:var(--bg)}
.btn-primary:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(0,212,170,.3)}
.btn-secondary{background:var(--bg);color:var(--text);border:1px solid var(--border)}
.btn-secondary:hover{border-color:var(--accent)}
.terminal-output{background:#000;border-radius:12px;padding:20px;margin-top:20px;font-family:'Consolas','Monaco',monospace;font-size:.85rem;line-height:1.6;color:#0f0;min-height:200px;max-height:500px;overflow-y:auto;border:1px solid #333;display:none}
...
.port-forward-item{display:flex;gap:10px;margin-bottom:10px;align-items:center}
.port-forward-item input{width:100px;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:8px;color:var(--text)}
.port-forward-item button{background:var(--red);color:white;border:none;border-radius:8px;padding:8px 15px;cursor:pointer}
.add-port-btn{background:var(--accent);color:var(--bg);border:none;border-radius:8px;padding:8px 20px;cursor:pointer;font-weight:bold}
.copy-btn{margin-top:10px;padding:10px 25px;background:var(--accent);color:var(--bg);border:none;border-radius:10px;cursor:pointer;font-weight:bold}
.copy-btn.copied{background:#0f0}
.run-btn{margin-top:10px;padding:12px 30px;background:linear-gradient(135deg,#00d4aa,#00b4d8);color:#0f0f1a;border:none;border-radius:10px;cursor:pointer;font-weight:bold;font-size:1rem;display:inline-flex;align-items:center;gap:8px}
.run-btn:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(0,212,170,.3)}
.run-btn:disabled{opacity:.5;cursor:not-allowed}
.spinner{display:inline-block;width:16px;height:16px;border:2px solid #0f0f1a;border-top:2px solid transparent;border-radius:50%;animation:spin 1s linear infinite}
@keyframes spin{100%{transform:rotate(360deg)}}
</style>
</head>
<body>
<div class="container">
<header><h1>🚀 Winbox VPS Manager</h1><p>Quản lý và tạo Windows VM chỉ với vài cú click</p></header>
<div class="status-bar">
<div class="status-pill"><span class="dot"></span><span id="status-text">Sẵn sàng</span></div>
<div class="status-pill">📍 Local: <span style="color:var(--accent)">localhost:'$WEB_PORT'</span></div>
<div class="status-pill">🖥️ Script: <span style="color:var(--accent)">winbox.sh</span></div>
</div>
<div class="section-title"><span>🪟</span> Chọn phiên bản Windows</div>
<div class="os-grid" id="osGrid">
<div class="os-card" data-os="win2012" data-uefi="no" data-user="administrator" data-pass="Tamnguyenyt@123"><div class="os-icon">🖥️</div><h3>Windows Server 2012 R2</h3><div class="os-desc">Server legacy, nhẹ, ổn định</div><span class="os-badge">SERVER</span></div>
<div class="os-card" data-os="win2022" data-uefi="no" data-user="administrator" data-pass="Tamnguyenyt@123"><div class="os-icon">🏢</div><h3>Windows Server 2022</h3><div class="os-desc">Server hiện đại, bảo mật cao</div><span class="os-badge">SERVER</span></div>
<div class="os-card" data-os="win11" data-uefi="yes" data-user="Admin" data-pass="Tam255Z"><div class="os-icon">🪟</div><h3>Windows 11 LTSB</h3><div class="os-desc">Windows 11 gọn nhẹ, không bloatware</div><span class="os-badge">DESKTOP</span></div>
<div class="os-card" data-os="win10ltsb" data-uefi="no" data-user="Admin" data-pass="Tam255Z"><div class="os-icon">💻</div><h3>Windows 10 LTSB 2015</h3><div class="os-desc">Windows 10 cổ điển, siêu nhẹ</div><span class="os-badge">LEGACY</span></div>
<div class="os-card" data-os="win10ltsc" data-uefi="no" data-user="Admin" data-pass="Tam255Z"><div class="os-icon">⚡</div><h3>Windows 10 LTSC 2023</h3><div class="os-desc">Phiên bản ổn định nhất, khuyên dùng</div><span class="os-badge" style="background:var(--yellow);color:#000">RECOMMENDED</span></div>
<div class="os-card" data-os="win10ltsb2022" data-uefi="no" data-user="Admin" data-pass="Tam255Z"><div class="os-icon">🎮</div><h3>Windows 10 LTSB 2022</h3><div class="os-desc">Hỗ trợ VirtGPU 3D, tốt cho gaming</div><span class="os-badge" style="background:var(--blue)">GAMING</span></div>
</div>
...
<div class="preset-card" data-cpu="1" data-ram="2" data-disk="20"><div class="preset-name">🥉 Basic</div><div class="preset-specs">1 vCPU<br>2 GB RAM<br>+20 GB Disk<br><span style="color:var(--dim)">Phù hợp: Lướt web, office</span></div></div>
<div class="preset-card" data-cpu="2" data-ram="4" data-disk="40"><div class="preset-name">🥈 Standard</div><div class="preset-specs">2 vCPU<br>4 GB RAM<br>+40 GB Disk<br><span style="color:var(--dim)">Phù hợp: Dev, multitask</span></div></div>
<div class="preset-card selected" data-cpu="4" data-ram="8" data-disk="60"><div class="preset-name">🥇 Pro</div><div class="preset-specs">4 vCPU<br>8 GB RAM<br>+60 GB Disk<br><span style="color:var(--dim)">Phù hợp: Heavy workload</span></div></div>
<div class="preset-card" data-cpu="8" data-ram="16" data-disk="100"><div class="preset-name">💎 Ultra</div><div class="preset-specs">8 vCPU<br>16 GB RAM<br>+100 GB Disk<br><span style="color:var(--dim)">Phù hợp: Gaming, render</span></div></div>
</div>
</div>
<div id="customTab" class="tab-content hidden">
<div class="custom-config">
<div class="input-group"><label>🖥️ Số CPU Cores</label><input type="number" id="customCpu" value="4" min="1" max="64"><span class="hint">Tối đa khuyến nghị: <span id="maxCpuHint">8</span></span></div>
<div class="input-group"><label>💾 RAM (GB)</label><input type="number" id="customRam" value="8" min="1" max="128"><span class="hint">Tối đa khuyến nghị: <span id="maxRamHint">16</span> GB</span></div>
<div class="input-group"><label>💽 Mở rộng Disk (GB)</label><input type="number" id="customDisk" value="40" min="0" max="500"><span class="hint">0 = không mở rộng</span></div>
<div class="input-group"><label>🆔 Instance ID</label><input type="number" id="instanceId" value="1" min="1" max="99"><span class="hint">RDP port = 3388 + ID</span></div>
</div>
<div class="checkbox-row"><input type="checkbox" id="safeDownload"><label for="safeDownload">📦 Safe Download (tải theo chunks 900MB)</label></div>
<div class="port-forwards"><label style="display:block;margin-bottom:10px;color:var(--dim)">🌐 Port Forwarding thêm (tùy chọn):</label><div id="portList"></div><button class="add-port-btn" onclick="addPortForward()">+ Thêm port</button></div>
</div>
<div class="auth-section"><h4>🔐 Thông tin đăng nhập RDP</h4><div class="auth-grid"><div class="input-group"><label>👤 Username</label><input type="text" id="rdpUser" value="Admin" readonly style="background:var(--bg);opacity:.7"></div><div class="input-group"><label>🔑 Password</label><input type="text" id="rdpPass" value="Tam255Z" readonly style="background:var(--bg);opacity:.7"></div></div><p style="margin-top:10px;font-size:.85rem;color:var(--dim)">ℹ️ Thông tin này được đặt theo image Windows đã chọn. Không thể thay đổi.</p></div>
<div class="summary-box"><h4>📋 Tóm tắt cấu hình</h4><div class="summary-item"><span>🪟 Hệ điều hành</span><span class="val" id="sumOs">-</span></div><div class="summary-item"><span>🖥️ CPU</span><span class="val" id="sumCpu">-</span></div><div class="summary-item"><span>💾 RAM</span><span class="val" id="sumRam">-</span></div><div class="summary-item"><span>💽 Disk mở rộng</span><span class="val" id="sumDisk">-</span></div><div class="summary-item"><span>🌐 RDP Port</span><span class="val" id="sumRdp">-</span></div><div class="summary-item"><span>⚡ Acceleration</span><span class="val" id="sumAccel">-</span></div><div class="summary-item"><span>📝 Lệnh sẽ chạy</span><span class="val" id="sumCmd" style="font-size:.8rem;word-break:break-all">-</span></div></div>
<div class="btn-group"><button class="btn btn-secondary" onclick="resetAll()">🔄 Chọn lại</button><button class="btn btn-primary" onclick="createVPS()" id="createBtn">🚀 TẠO VPS NGAY</button></div>
</div>
<div class="terminal-output" id="terminalOutput"></div>
</div>
<script>
let selectedOs=null,selectedPreset=null,currentTab='preset',isRunning=false;
document.querySelectorAll('.os-card').forEach(card=>{card.addEventListener('click',()=>{document.querySelectorAll('.os-card').forEach(c=>c.classList.remove('selected'));card.classList.add('selected');selectedOs={name:card.querySelector('h3').textContent,flag:card.dataset.os,uefi:card.dataset.uefi,user:card.dataset.user,pass:card.dataset.pass};document.getElementById('selectedOsName').textContent=selectedOs.name;document.getElementById('configPanel').classList.add('active');document.getElementById('rdpUser').value=selectedOs.user;document.getElementById('rdpPass').value=selectedOs.pass;updateSummary();document.getElementById('configPanel').scrollIntoView({behavior:'smooth'})})});
document.querySelectorAll('.preset-card').forEach(card=>{card.addEventListener('click',()=>{document.querySelectorAll('.preset-card').forEach(c=>c.classList.remove('selected'));card.classList.add('selected');selectedPreset={cpu:card.dataset.cpu,ram:card.dataset.ram,disk:card.dataset.disk};updateSummary()})});
function switchTab(tab){currentTab=tab;document.querySelectorAll('.config-tab').forEach(t=>t.classList.remove('active'));event.target.classList.add('active');if(tab==='preset'){document.getElementById('presetTab').classList.remove('hidden');document.getElementById('customTab').classList.add('hidden')}else{document.getElementById('presetTab').classList.add('hidden');document.getElementById('customTab').classList.remove('hidden')}updateSummary()}
function addPortForward(){const div=document.createElement('div');div.className='port-forward-item';div.innerHTML='<input type="number" placeholder="Host" class="pf-host" value="8080"><span>→</span><input type="number" placeholder="Guest" class="pf-guest" value="80"><button onclick="this.parentElement.remove();updateSummary();">✕</button>';document.getElementById('portList').appendChild(div)}
function getConfig(){let cpu,ram,disk,instanceId;if(currentTab==='preset'&&selectedPreset){cpu=selectedPreset.cpu;ram=selectedPreset.ram;disk=selectedPreset.disk;instanceId=1}else{cpu=document.getElementById('customCpu').value;ram=document.getElementById('customRam').value;disk=document.getElementById('customDisk').value;instanceId=document.getElementById('instanceId').value}const ports=[];document.querySelectorAll('.port-forward-item').forEach(item=>{const h=item.querySelector('.pf-host').value;const g=item.querySelector('.pf-guest').value;if(h&&g)ports.push(h+':'+g)});return{cpu,ram,disk,instanceId,ports}}
function updateSummary(){if(!selectedOs)return;const cfg=getConfig();const rdpPort=3388+parseInt(cfg.instanceId);document.getElementById('sumOs').textContent=selectedOs.name;document.getElementById('sumCpu').textContent=cfg.cpu+' cores';document.getElementById('sumRam').textContent=cfg.ram+' GB';document.getElementById('sumDisk').textContent=cfg.disk+' GB';document.getElementById('sumRdp').textContent='localhost:'+rdpPort;document.getElementById('sumAccel').textContent='Auto-detect (KVM/TCG)';let cmd='bash winbox.sh --auto --'+selectedOs.flag;if(currentTab==='custom')cmd='export WINBOX_VCPUS='+cfg.cpu+' WINBOX_RAM_GB='+cfg.ram+' && '+cmd;if(cfg.disk>0)cmd+=' && bash winbox.sh --resize=+'+cfg.disk+'G --id='+cfg.instanceId;if(cfg.instanceId>1)cmd+=' --id='+cfg.instanceId;cfg.ports.forEach(p=>cmd+=' --port-forward='+p);if(document.getElementById('safeDownload')?.checked)cmd+=' --safe-download';document.getElementById('sumCmd').textContent=cmd}
document.querySelector('.preset-card[data-cpu="4"]').click();
['customCpu','customRam','customDisk','instanceId','safeDownload'].forEach(id=>{const el=document.getElementById(id);if(el)el.addEventListener('change',updateSummary)});
function resetAll(){document.querySelectorAll('.os-card').forEach(c=>c.classList.remove('selected'));document.getElementById('configPanel').classList.remove('active');selectedOs=null;document.getElementById('terminalOutput').classList.remove('active');window.scrollTo({top:0,behavior:'smooth'})}
function createVPS(){if(!selectedOs){alert('Vui lòng chọn phiên bản Windows trước!');return}if(isRunning)return;const cfg=getConfig();const rdpPort=3388+parseInt(cfg.instanceId);const term=document.getElementById('terminalOutput');term.classList.add('active');term.innerHTML='';function log(msg,type='info'){const colors={cmd:'#ff0',info:'#0af',success:'#0f0',error:'#f00',warn:'#fa0'};term.innerHTML+='<div style="color:'+(colors[type]||colors.info)+'">'+msg+'</div>';term.scrollTop=term.scrollHeight}log('=== WINBOX VPS MANAGER ===','success');log('🪟 OS: '+selectedOs.name,'info');log('🖥️ CPU: '+cfg.cpu+' cores | RAM: '+cfg.ram+'GB | Disk: +'+cfg.disk+'GB','info');log('🌐 RDP: localhost:'+rdpPort,'info');log('👤 User: '+selectedOs.user+' | Pass: '+selectedOs.pass,'info');log('','info');let flags='--auto --'+selectedOs.flag;let envPrefix='';if(currentTab==='custom'){envPrefix='export WINBOX_VCPUS='+cfg.cpu+' WINBOX_RAM_GB='+cfg.ram+' && '}if(cfg.instanceId>1)flags+=' --id='+cfg.instanceId;cfg.ports.forEach(p=>flags+=' --port-forward='+p);if(document.getElementById('safeDownload')?.checked)flags+=' --safe-download';const mainCmd=envPrefix+'bash winbox.sh '+flags;log(mainCmd,'cmd');if(cfg.disk>0){const resizeCmd='bash winbox.sh --resize=+'+cfg.disk+'G --id='+cfg.instanceId;log(resizeCmd,'cmd')}log('','info');let fullCmd=mainCmd;if(cfg.disk>0)fullCmd+='\n'+('bash winbox.sh --resize=+'+cfg.disk+'G --id='+cfg.instanceId);const btnGroup=document.createElement('div');btnGroup.style.cssText='display:flex;gap:12px;margin-top:12px;flex-wrap:wrap;';const copyBtn=document.createElement('button');copyBtn.textContent='📋 Copy to Clipboard';copyBtn.className='copy-btn';copyBtn.onclick=()=>{navigator.clipboard.writeText(fullCmd).then(()=>{copyBtn.textContent='✅ Đã copy!';copyBtn.classList.add('copied');setTimeout(()=>{copyBtn.textContent='📋 Copy to Clipboard';copyBtn.classList.remove('copied')},2000)})};const runBtn=document.createElement('button');runBtn.innerHTML='▶️ Chạy trực tiếp';runBtn.className='run-btn';runBtn.onclick=()=>{if(isRunning)return;isRunning=true;runBtn.disabled=true;runBtn.innerHTML='<span class="spinner"></span> Đang chạy...';log('⏳ Đang gửi lệnh tới server...','warn');fetch('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({os:selectedOs.flag,cpu:cfg.cpu,ram:cfg.ram,disk:parseInt(cfg.disk),id:parseInt(cfg.instanceId),ports:cfg.ports,safeDownload:document.getElementById('safeDownload')?.checked||false,tab:currentTab})}).then(r=>r.json()).then(data=>{if(data.job_id){log('✅ Job đã bắt đầu (background)','success');log('📡 Polling output mỗi 3 giây...','info');const poll=()=>{fetch('/api/output?job='+data.job_id).then(r=>r.json()).then(d=>{if(d.output){term.innerHTML=term.innerHTML.replace(/⏳ Đang gửi lệnh tới server\.{0,3}/g,'');const marker='<div id="poll-marker"></div>';term.innerHTML=term.innerHTML.replace(marker,'')+marker;const markerEl=document.getElementById('poll-marker');if(markerEl){markerEl.outerHTML='<pre style="color:#0f0;font-size:.8rem;white-space:pre-wrap;margin:0">'+escapeHtml(d.output)+'</pre>'}term.scrollTop=term.scrollHeight}if(d.running){setTimeout(poll,3000)}else{isRunning=false;runBtn.disabled=false;runBtn.innerHTML='▶️ Chạy trực tiếp';if(d.returncode!==0&&d.returncode!==null){log('❌ Job failed (exit '+d.returncode+')','error')}else{log('✅ Job hoàn tất!','success')}}}).catch(e=>{isRunning=false;runBtn.disabled=false;runBtn.innerHTML='▶️ Chạy trực tiếp';log('❌ Lỗi poll: '+e.message,'error')})};poll()}else{isRunning=false;runBtn.disabled=false;runBtn.innerHTML='▶️ Chạy trực tiếp';if(data.success){log('✅ Lệnh đã được thực thi!','success');if(data.output){log('<pre style="color:#0f0;font-size:.8rem;white-space:pre-wrap;">'+escapeHtml(data.output)+'</pre>','info')}}else{log('❌ Lỗi: '+(data.error||'Không rõ'),'error');if(data.details)log('📋 Chi tiết: '+escapeHtml(data.details),'warn')}}}).catch(err=>{isRunning=false;runBtn.disabled=false;runBtn.innerHTML='▶️ Chạy trực tiếp';log('❌ Lỗi kết nối: '+err.message,'error');log('💡 Gợi ý: Copy lệnh và chạy thủ công trong terminal','warn')})};btnGroup.appendChild(copyBtn);btnGroup.appendChild(runBtn);term.appendChild(btnGroup);navigator.clipboard.writeText(fullCmd).catch(()=>{});document.getElementById('createBtn').textContent='✅ Đã tạo lệnh!';setTimeout(()=>document.getElementById('createBtn').textContent='🚀 TẠO VPS NGAY',3000);term.scrollIntoView({behavior:'smooth'})}
function escapeHtml(text){const div=document.createElement('div');div.textContent=text;return div.innerHTML}
function detectResources(){const cores=navigator.hardwareConcurrency||4;const mem=navigator.deviceMemory||8;document.getElementById('maxCpuHint').textContent=cores;document.getElementById('maxRamHint').textContent=mem;document.getElementById('customCpu').max=cores;document.getElementById('customRam').max=mem}
detectResources();
</script>
</body>
</html>
HTMLEOF
    ...
    echo -e "${G}✔${W}  Đã tạo giao diện web"
    ...
    python3 - "$WEB_PORT" "$WEB_DIR" "$SCRIPT_DIR" "$SCRIPT_NAME" <<'PYEOF'
import sys, os, subprocess, threading, time, json, socketserver, http.server

WEB_PORT = int(sys.argv[1])
WEB_DIR = sys.argv[2]
SCRIPT_DIR = sys.argv[3]
SCRIPT_NAME = sys.argv[4]
WINBOX_PATH = os.path.join(SCRIPT_DIR, SCRIPT_NAME)
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/status':
            self._json(200, {
                ...
                'winbox_exists': os.path.exists(WINBOX_PATH),
                'jobs': {k: {'running': v['proc'].poll() is None, 'start_time': v['start_time']}
                        for k, v in BG_JOBS.items()}
            })
        elif self.path.startswith('/api/output'):
            job_id = ''
            if 'job=' in self.path:
                job_id = self.path.split('job=')[-1].split('&')[0]
            if job_id in BG_JOBS:
                logfile = BG_JOBS[job_id]['logfile']
                proc = BG_JOBS[job_id]['proc']
                out = ''
    ...
    def do_POST(self):
        if self.path == '/api/run':
            ...
            os_flag = data.get('os', 'win10ltsc')
            cpu = data.get('cpu', 4)
            ram = data.get('ram', 8)
            disk = int(data.get('disk', 0))
            inst_id = data.get('id', 1)
            ports = data.get('ports', [])
            safe_dl = data.get('safeDownload', False)
            tab = data.get('tab', 'preset')

            if not os.path.exists(WINBOX_PATH):
                self._json(500, {
                    'success': False,
                    'error': 'Khong tim thay winbox.sh',
                    'details': f'Duong dan: {WINBOX_PATH}'
                })
                return

            cmd_parts = ['bash', WINBOX_PATH, '--auto', f'--{os_flag}']
            env = os.environ.copy()
            if tab == 'custom':
                env['WINBOX_VCPUS'] = str(cpu)
                env['WINBOX_RAM_GB'] = str(ram)

            if inst_id > 1:
                cmd_parts.extend(['--id', str(inst_id)])
            for p in ports:
                cmd_parts.extend(['--port-forward', p])
            if safe_dl:
                cmd_parts.append('--safe-download')

            job_id = f'job_{os.getpid()}_{int(time.time())}'
            logfile = f'/tmp/winbox-web-{job_id}.log'

            with open(logfile, 'w') as f:
                f.write(f'=== WINBOX JOB {job_id} ===\n')
                f.write(f'CMD: {" ".join(cmd_parts)}\n')
            ...
            def run_bg():
                try:
                    with open(logfile, 'a') as out_f:
                        ...
                        proc.wait()
                        if disk > 0 and proc.returncode == 0:
                            with open(logfile, 'a') as out_f2:
                                out_f2.write('\n--- RESIZE ---\n')
                                out_f2.flush()
                            resize_cmd = ['bash', WINBOX_PATH, f'--resize=+{disk}G', f'--id={inst_id}']
                            with open(logfile, 'a') as out_f2:
            ...
            if job_id in BG_JOBS:
                ...
                if proc.poll() is not None and proc.poll() != 0:
                    ...
                    self._json(200, {
                        'success': False,
                        'error': f'Command failed immediately (exit {proc.returncode})',
                        'output': out,
                        'job_id': job_id
                    })
                    return
            ...
            self._json(200, {
                'success': True,
                'output': f'Job started. Polling /api/output?job={job_id}',
                'job_id': job_id,
                'poll_url': f'/api/output?job={job_id}'
            })
        else:
...
with socketserver.TCPServer(('127.0.0.1', WEB_PORT), Handler) as httpd:
    print(f'Server: http://127.0.0.1:{WEB_PORT}')
    print(f'Winbox: {WINBOX_PATH}')
    httpd.serve_forever()
PYEOF
    SERVER_PID=$!

    echo -e "${G}✔${W}  Server đang chạy (PID: $SERVER_PID)"
    echo -e "${Y}⚠${W}  Nhấn Ctrl+C để dừng"
